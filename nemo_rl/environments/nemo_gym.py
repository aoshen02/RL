    ...
    for item in response_input:
        ...
        for part in content:
            ...
            if part_type == "input_text":
                ...
                raise ValueError(
                    "The initial Gym video contract does not support mixing "
                    "still images and video in one row."
                )
...
def nemo_gym_example_to_video_datum_spec(
    ...
    *,
    ...
    The raw video remains in the outbound Gym request. The policy-side copy is
    decoded with the configured vLLM loader and converted into frame tensors;
    those tensors are reattached to vLLM-authored prompt token IDs after the
    rollout.
    """
    ...
    if type(processor).__name__ not in _NEMOTRON_VIDEO_PROCESSOR_NAMES:
