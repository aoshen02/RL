...
def setup(
    ...
    if colocated_inference:
        ...
        if use_server_groups:
            ...
            base_vllm_cfg = generation_config["vllm_cfg"]
            inference_clusters: dict[str, RayVirtualCluster] = {}
            for group in server_groups:
                group_name, group_gpus = group["name"], group["gpus"]
                if group_gpus <= inference_gpus_per_node:
                    bundles = [group_gpus]
                else:
                    assert group_gpus % inference_gpus_per_node == 0, (
                        f"server_group '{group_name}': cross-node groups must be "
                        f"whole-node multiples of {inference_gpus_per_node} GPUs"
                    )
                    bundles = [inference_gpus_per_node] * (
                        group_gpus // inference_gpus_per_node
                    )
                    group_vllm_cfg = (group.get("overrides") or {}).get("vllm_cfg", {})
                    model_parallel = group_vllm_cfg.get(
                        "tensor_parallel_size", base_vllm_cfg["tensor_parallel_size"]
                    ) * group_vllm_cfg.get(
                        "pipeline_parallel_size",
                        base_vllm_cfg["pipeline_parallel_size"],
                    )
                    if model_parallel <= inference_gpus_per_node:
                        assert inference_gpus_per_node % model_parallel == 0, (
                            f"server_group '{group_name}': TP*PP={model_parallel} must "
                            f"divide gpus_per_node={inference_gpus_per_node}"
                        )
                inference_clusters[group_name] = RayVirtualCluster(
                    name=f"grpo_inference_{group_name}",
                    bundle_ct_per_node_list=bundles,
                    use_gpus=True,
                    num_gpus_per_node=bundles[0],
                    max_colocated_worker_groups=1,
                    port_range_low=cluster_config.get("master_port_range_low"),
                    port_range_high=cluster_config.get("master_port_range_high"),
                )
                print(
                    ...
                    f"initialized with bundles {bundles}",
                    flush=True,
                )
