...
def _should_use_nemo_gym(master_config: MasterConfig) -> bool:
    ...
    assert _should_use_async_rollouts(master_config), (
        "❌ Error: In order to use NeMo-Gym, use Dynamo or a generation "
        "backend with async rollouts enabled."
    )
    ...
    if generation_config["backend"] == "vllm":
        ...
        # DynamoGeneration always exposes its managed frontend or local
