...
class MultiVllmGeneration(GenerationInterface):
    ...
    def __init__(
        ...
        assert clusters.keys() == configs.keys(), (
            f"cluster/config group names mismatch: {set(clusters)} vs {set(configs)}"
        )
        self.groups = {
            name: VllmGeneration(
                clusters[name],
    ...
    @property
    def server_group_urls(self) -> dict[str, list[Optional[str]]]:
        return {n: list(g.dp_openai_server_base_urls) for n, g in self.groups.items()}
    ...
    def invalidate_kv_cache(self) -> bool:
        return all([g.invalidate_kv_cache() for g in self.groups.values()])
    ...
    def snapshot_step_metrics(self) -> None:
        for g in self.groups.values():
            g.snapshot_step_metrics()

    def get_step_metrics(self) -> dict[str, float]:
        return {
            f"{name}/{k}": v
            for name, g in self.groups.items()
            for k, v in g.get_step_metrics().items()
        }
