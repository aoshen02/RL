...
def _load_video_frames_torchcodec_with_metadata(
    ...
    def sample_indices(metadata: Any, **_: Any) -> np.ndarray:
    ...
    metadata["frames_indices"] = [
        int(frame_idx) for frame_idx in metadata.get("frames_indices", [])
    ]
    metadata["video_sampling_style"] = _get_video_sampling_style()
    ...
    class TorchCodecNemotronVLVideoBackend:
        ...
        def load_bytes(
            ...
            **kwargs: Any,
            ...
            sampled_indices = [
                _timestamp_to_video_frame_index(timestamp, source_fps, total_frames)
                for timestamp in timestamps_s
            ]
