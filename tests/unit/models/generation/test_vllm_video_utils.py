...
def test_sync_post_init_initializes_folded_radio_layerscale_before_numa_binding():
    ...
    assert worker.llm.collective_rpc.call_args_list == [
        call("_initialize_nemotron_omni_radio_layerscale", args=tuple()),
...
def test_torchcodec_loader_uses_nemotron_sampling_and_returns_rgb(monkeypatch):
    ...
    def fake_load_video(video_path, *, backend, sample_indices_fn):
        metadata = SimpleNamespace(total_num_frames=20, fps=2.0)
        indices = sample_indices_fn(metadata)
        captured.update(
            video_path=video_path,
            backend=backend,
            indices=indices.tolist(),
        )
        frames = torch.zeros((len(indices), 3, 2, 4), dtype=torch.uint8)
        return frames, {
            "fps": 2.0,
            "total_num_frames": 20,
    ...
    monkeypatch.setattr("transformers.video_utils.load_video", fake_load_video)
